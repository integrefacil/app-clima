...
export function DailyList({ days, compact }: { days: Day[]; compact?: boolean }) {
  const { t, i18n } = useTranslation()
  const visible = compact ? days.slice(0, 2) : days
  ...
  return (
    <div className={compact ? 'p-1' : 'glass p-3 md:p-4'}>
      {!compact && <p className="text-xs md:text-sm text-white/60 uppercase tracking-widest mb-2">{t('daily.title')}</p>}
      {compact && <p className="text-xs text-white/60 mb-1">{t('daily.title')}</p>}
      <div>
        {visible.map((d, i) => {
          ...
          return (
            <div key={i}>
              <div className="flex items-center gap-2 py-2.5 text-sm md:text-base">
                <span className="w-28 md:w-32 text-white/90 truncate" title={d.label}>{d.label}</span>
              ...
              {i < visible.length - 1 && <Separator.Root className="h-px bg-white/10" />}
            </div>
          )
        })}
      </div>
